import { OrderAuthService } from '../../../src/payments/order-auth.service';
...
describe('OrderAuthService', () => {
  ...
  it('rejects a missing api key or signature without hitting the datastore (401, not a 500)', async () => {
    const findUnique = jest.fn();
    const prisma = { merchantApiKey: { findUnique } } as any;
    const svc = new OrderAuthService(prisma, apiKeys as any, { open: jest.fn(), seal: jest.fn() } as any);
    await expect(svc.verify(undefined as any, '{}', 'sig')).rejects.toThrow();
    await expect(svc.verify('navy_pk_x', '{}', undefined as any)).rejects.toThrow();
