...
import { RelayerService } from '../../../src/payments/relayer.service';
import { buildAuthorizationTypedData, merchantIdHex, invoiceIdHexFromOrderId, invoiceKey, authorizationDigest, type UsdcDomain } from '../../../src/evm/payment-authorization';
...
describe('RelayerService (EVM)', () => {
  ...
  it('verifyAndSubmit happy path: recovers payer, consumes atomically before submit, returns {txHash,payer,err:null}', async () => {
    ...
    // payInvoice called with (merchantIdHex, invoiceIdHex, amount, validAfter=0, validBefore, payer, v, r, s) — 9 args
    ...
    expect(args[4]).toBe(Math.floor(expiresAt.getTime() / 1000));
    expect(args[5]).toBe(wallet.address);
    const sigParts = ethers.Signature.from(sig);
    expect(args[6]).toBe(sigParts.v);
    expect(args[7]).toBe(sigParts.r);
    expect(args[8]).toBe(sigParts.s);
  });
