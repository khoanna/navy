...
**Development is spec-driven.** Features are designed → planned → built via `docs/superpowers/specs/*-design.md` + `docs/superpowers/plans/*.md`; read the relevant one before changing a subsystem, since they hold the *why* and the locked decisions. **Only the still-active specs/plans remain** — obsolete ones were deleted (commit `7127ce2`, and the app-specific ones on 2026-09-13), so historical designs exist **only in git history**: `git log --diff-filter=D --name-only -- docs/superpowers` to find one, `git show <commit>:<path>` to read it. List the live ones with `ls docs/superpowers/{specs,plans}`. The line runs: paper conformance (2026-09-07/08, phases 1–4 complete) → v0.7 movement rule and release readiness (2026-09-09) → P36 hurdle bound (2026-09-13). `plans/2026-09-07-srcla-phase1-outcome.md` holds the operator runbook.

Other durable design docs: `docs/research/output/srcla-paper.md` (the algorithm), `docs/research/` (supporting research), `docs/PRODUCTION.md`, `contract/DEPLOYMENTS.md`, `contract/script/POST_DEPLOY.md`, `contract/audit/`.
...
pnpm exec tsc --noEmit            # typecheck src/  (pnpm typecheck:scripts covers scripts/)
...
- **`docs/` is in `.gitignore` and untracked (2026-09-13).** Its specs, plans, research and the paper live only on the local disk — a fresh clone has none of them, and versions from before 2026-09-13 exist only in git history (`git show <commit>:docs/<path>`).
- **Production gates + accepted pre-production risks:** `docs/PRODUCTION.md`. New work: branch off `main`.

**Anvil fork:** see *Running locally* above for the fork + deploy commands. `contract/script/` holds the real set — there is **no** `DeployVaultAnvil.s.sol`, and since the stub and the payment gateway were removed, no `DeploySimpleAnvil.s.sol`/`SetupAnvil.s.sol`/`DeployFull.s.sol`/`DeployDirect.s.sol`/`Deploy.s.sol` either.

**srcla Database:** Postgres 16 on port 5433, via `srcla/docker-compose.yml`. `DATABASE_URL=postgresql://user:password@localhost:5433/srcla`. Run `pnpm prisma:push` after schema changes.

**SRCLA Paper & evaluation.** Algorithm spec: `docs/research/output/srcla-paper.md` — **v0.10**, carrying Amendment Records P1–P36 and three burned-window declarations; both are binding spec, not proposals. The registered evaluation has been RUN on both sealed eras and returns `FAIL` (published, not tuned away); §13 and Appendix F of the paper hold the results and the disclosures. Defines baselines B0–B5 (incl. B2u), ablations **H1–H7**, and two release gates (forecast calibration + policy outperformance). Evaluation report: `report/SRCLA-REPORT.md` (+ `SRCLA-REPORT.json` at the root). `srcla/scripts/run-live-evaluation.ts` is a hard-coded illustration (fixed market numbers; it never reads the fork) — the registered evaluation is `pnpm phase4:run`. `scripts/show-live-apys.ts` prints the venue APYs the fork is currently reporting.
...
**There IS held-out data now.** Phase 4 backfilled **21,769 hourly origins from
2024-03-15** of Base mainnet venue state (Compound III, Aave V3, Moonwell) with
...
venues reads back to the deployment floor (block 11,707,031, 2024-03-12), and this
repository had never looked at any of it, so the "no held-out data" blocker is closed by collecting rather than by
waiting.
