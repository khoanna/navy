...
- **⚠️ MUST live-smoke-test before relying on auto-farm (unverifiable from code):** the client uses `useHeadlessDelegatedActions().delegateWallet` (Privy "delegated actions", Mechanism A) + a dashboard-registered authorization key on the server (`PRIVY_AUTHORIZATION_KEY`). It is **not provable from the SDK types/docs alone** whether `delegateWallet` *by itself* authorizes our specific dashboard key to sign, or whether an explicit `useSigners().addSigners({ signers:[{ signerId, policyIds }] })` with the key-quorum id is also required (Mechanism B). If Mechanism A is insufficient, every delegated fund throws and is now audited as `farming.delegated.fund.error` (agent) / surfaced as an error (fund-now) — check that audit action after the first real enable+fund. **Verification:** on a devnet dev-client, enable auto-farm, then confirm a `farming.delegated.fund` (not `.error`) audit row + an on-chain user→subwallet transfer. If you see `.error`, wire Mechanism B (register a key quorum in the dashboard, store its id, call `addSigners` in the enable flow).
- Flow hardening from the 2026-07-12 logic audit: `enable` retries the Privy delegated-wallet lookup (0/600/1500ms) to absorb write-then-read propagation lag; address-fallback signing (walletId absent) is logged; delegated-sign failures are audited.
